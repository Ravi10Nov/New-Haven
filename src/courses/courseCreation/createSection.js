...
export default function CreateSection({ url }) {
  ...
  const [sectionDeleteModalOpen, setSectionDeleteModalOpen] =
    React.useState(false);
  const [isSectionDeleted, setIsSectionDeleted] = useState(false);
  const [selectedSectionId, setSelectedSectionId] = useState(null);
  const [isSectionsOrderUpdated, setIsSectionsOrderUpdated] = useState(false);
  ...
  const handleUpdateSectionsOrder = async () => {
    ...
    console.log(updatedSectionsOrder);
    ...
    try {
      ...
      console.error(error);
      toast.error("Failed to update sections order 2");
    }
  };
  ...
  const handleSectionDelete = async () => {
    const response = await axios.delete(
      `${process.env.REACT_APP_BACKEND_URL}/api/courses/section/${selectedSectionId}`
    );
    setSectionDeleteModalOpen(false);
    setIsSectionDeleted(!isSectionDeleted);
  ...
  useEffect(() => {
    fetch();
  }, [isNewSectionCreated, isSectionDeleted, isSectionsOrderUpdated]);
      ...
      <DragDropContext onDragEnd={moveSection}>
        <div style={{ backgroundColor: "white" }}>
          ...
          <div className="flex flex-col items-center justify-center mt-8">
            ...
            <div className="flex items-center justify-end">
              {/* <Button variant="contained" color="success" onClick={handleUpdateSectionsOrder}>
              Save Sections Order
            </Button> */}
              <button
                ...
                onClick={handleUpdateSectionsOrder}
              >
                Save Sections Order
              </button>
            </div>
